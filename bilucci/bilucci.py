...
# Spoils list
SPOILS = ["No spoilers for the moment"]
...
def start_spoiling(update, context):
    ...
    spoil = choice(SPOILS)
    context.bot.send_message(chat_id=update.effective_chat.id, text=spoil)
...
def good_night_alphabit(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text=good_night_msg)
...
def esi_cinema(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text=esi_cinema_msg)
...
def help_cmd(update, context):
    ...
    commands = [
        ...
        ["/good_night_alphabit", "Say Good Night to Alphabit"],
        ["/good_night_ingeniums", "Say Good Night to Ingeniums"],
...
def main():
    ...
    updater.start_webhook(listen="0.0.0.0", port=PORT, url_path=BOT_API_TOKEN)
